...
def scrape():
    devpost_domain = request.form.get('devpost_domain')
    ...
    # Save to CSV
    csv_filename = f"{devpost_domain}_projects.csv"
    df = pd.DataFrame(all_projects)
    df.to_csv(csv_filename, index=False)

    # Read CSV data
    csv_data = df.to_html(classes="table-auto")
    ...
    return render_template('result.html', num_projects=len(all_projects), filename=csv_filename, csv_data=csv_data)
